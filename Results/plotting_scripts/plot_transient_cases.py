...
def plot_characteristic_time_evolution_paper():
    ...
    fig, axs = plt.subplots(nrows=2, ncols=1, sharex=True, figsize=([6.4, 4.8]))
    ...
    # dpa_values = dpa_valuses / year
    for T, char_times, colour in zip(T_values, characteristic_times, colours):
        axs[0].plot(dpa_values, char_times, color=colour)
...
def plot_transient_inventories_varying_T_and_damage_24h_alt():
    ...
    reference_inventories = np.loadtxt("reference_inventories_24h")
    normalised_inventories = np.loadtxt("normalised_inventories_24h")
    inventories = np.loadtxt("inventories_24h")
    inventories_paper = np.loadtxt("inventories_24h_paper")

    # plt.figure()

    # for case, colour in zip(inventories, colours):
    #     plt.plot(T_values, case, color=colour)

    # # plt.plot(T_values, reference_inventories, color="black")

    # plt.ylabel(r"T retention (m$^{-3}$)")
    # plt.xlabel(r"Temperature (K)")
    # plt.ylim(1e16, 1e23)
    # plt.xlim(400, 1300)
    # plt.yscale("log")
    ...
    # print(np.max(normalised_inventories_alt))
    # print(1e04)
    # quit()
    ...
    ax.clabel(CS2, inline=True, fontsize=10, fmt="%.0e")
    ...
    plt.yscale("log")
    plt.ylabel(r"Damage rate (dpa fpy$^{-1}$)")
    plt.xlabel(r"Temperature (K)")
    plt.tight_layout()


def plot_retention_profile_24h():
    dpa_values = np.geomspace(1e-05, 1e03, num=9)
    ...
    ax.spines["top"].set_visible(False)
    plt.subplots_adjust(wspace=0.112, hspace=0.071)
    plt.colorbar(sm, label=r"Damage rate (dpa fpy$^{-1}$)")
    plt.tight_layout()


def plot_transient_inventories_varying_T_and_no_damage():

    T_values = np.linspace(400, 1300, num=50)
    sim_times = [1e02, 1e03, 1e04, 1e05, 1e06]
    results_folder = "../parametric_studies/case_24h/"

    reference_inventories = []
    for time in sim_times:
        results_per_sim_time = []
        for T in T_values:
            no_damage_results_file = (
                results_folder
                + "dpa=0/time={:.1e}/T={:.0f}/derived_quantities.csv".format(time, T)
            )
            no_damage_data = np.genfromtxt(
                no_damage_results_file, delimiter=",", names=True
            )
            results_per_sim_time.append(no_damage_data["Total_retention_volume_1"][-1])
        reference_inventories.append(results_per_sim_time)

    plt.figure()

    for case, time in zip(reference_inventories, sim_times):
        plt.plot(T_values, case, label="{:.1e}".format(time))
...
# plot_characteristic_time_evolution_paper()
...
plot_retention_profile_24h()
# plot_transient_inventories_varying_T_and_no_damage()
